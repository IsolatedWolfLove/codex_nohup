...
function TerminalView({ item, active }: { item: OpenTerminal; active: boolean }) {
  ...
  useEffect(() => {
    ...
    const input = instance.onData((data) => void window.electronAPI.writeTerminal(item.id, data));
    const unsubscribe = window.electronAPI.onTerminalEvent((event: TerminalEvent) => {
      if (event.terminalId !== item.id) return;
      if (event.type === 'data') instance.write(event.data);
      if (event.type === 'error') instance.writeln(`\r\n\x1b[31m${event.message}\x1b[0m`);
      if (event.type === 'exit') instance.writeln('\r\n\x1b[90m[连接已断开，会话仍在远端运行]\x1b[0m');
    });
    void window.electronAPI.readyTerminal(item.id).then((buffered) => {
      if (buffered) instance.write(buffered);
    });
    const observer = new ResizeObserver(() => {
      addon.fit();
      void window.electronAPI.resizeTerminal(item.id, instance.cols, instance.rows);
    });
...
export function App() {
  ...
  const refresh = useCallback(async () => {
    if (!connected) return;
    try { setSessions(await window.electronAPI.listSessions()); setMessage(''); }
  ...
  async function connect(event: React.FormEvent) {
    ...
    try {
      const info = await window.electronAPI.connect({ host, port, username, authMethod, password, privateKeyPath, passphrase });
  ...
  async function open(name: string) {
    ...
    try {
      const result = await window.electronAPI.openSession({ name, cwd });
  ...
  async function kill(name: string) {
    if (!window.confirm(`结束远端会话“${name}”？其中运行的进程也会结束。`)) return;
    try { await window.electronAPI.killSession(name); await refresh(); }
    catch (error) { setMessage(errorMessage(error)); }
  }

  async function closeTerminal(id: string) {
    await window.electronAPI.closeTerminal(id).catch(() => undefined);
  ...
  async function disconnect() {
    await window.electronAPI.disconnect(); setConnected(null); setSessions([]); setTerminals([]); setActiveId(null); setMessage('已断开；持久会话仍在服务器运行。');
  }
